...
<@c.Box style="border-bottom:none;border-left:none;border-right:none;box-shadow:none;">
    <@c.BoxBody  style="width:100%;max-height:560px;min-height:330px; ">    	
        ...
        <@c.FormInput name="name" label="任务名称" validation={"alphanumerSpec":true,"required":true,"minlength":1,"maxlength":128} helper="1-128位字符,仅支持汉字、字母、数字和部分特殊字符（.- _()[]（）【】）组合" events="{blur :function(){$Utils.validInputSpeChar(this)}}"/>
        <@c.Hidden name="group" value="${jobdef?if_exists.group?if_exists}"/>
        <@c.Hidden name="jobGroup" value="${jobdef?if_exists.group?if_exists}"/>
        <@c.FormText name="description" label="任务描述" validation={"maxlength":250}></@c.FormText>
        <@c.FormInput name="jobName" label="任务类型" value="${jobdef?if_exists.name?if_exists}" readonly=true/>
        <@c.FormItem id="triggerState" label="状态" helper="部署任务是否生效"  >
