...
<@c.Box>
    <@c.BoxHeader class="border header-bg">
        <h3 class="box-title" style="font-size:14px;">预警信息</h3>
    </@c.BoxHeader>
    <@c.BoxBody>
    <@c.Form  id="jobForm" action="" method="post" after={"switchtab":"#mainPanel","dataloader":"#jobdefsContent"}>
    	<@c.FormGroup id="common">
	        <@c.FormInput name="name" label="预警名称" value="${job?if_exists.name?if_exists}" readonly=true/>
	        <@c.FormText name="description" label="预警描述" readonly=true> ${job?if_exists.description?if_exists}</@c.FormText>
	        <@c.FormInput name="jobName" label="预警类型" value="${job?if_exists.jobName?if_exists}" readonly=true/>
	    </@c.FormGroup>
        ...
        <#if parameters??&& parameters?size gt 0>
	        <@c.FormGroup id="para" label="预警条件:">
				 <@c.FormItem>
					<#include "/gdsp/page/schedule/pub/paramShow-list.ftl">
				 </@c.FormItem> 
	        </@c.FormGroup>
        </#if>   
        <@c.FormGroup id="para" label="预警:">
    		<@c.FormText name="messageinf" label="预警内容" readonly=true>${job?if_exists.messageinf?if_exists}</@c.FormText>
