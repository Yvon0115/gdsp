...
<@c.Box>
    <@c.BoxHeader class="border header-bg">
        <h3 class="box-title"><#if editType??&& editType=="edit">修改预警类型<#else>添加预警类型</#if></h3>
    </@c.BoxHeader>
    <@c.BoxBody>
    <@c.Form  id="alertdefForm" class="validate" action="${ContextPath}/schedule/alertdef/save.d" method="post" after={"switchtab":"#mainPanel","dataloader":"#alertdefsContent"}>
        <#if editType??&&editType=="edit">
        <@c.FormInput name="name" label="名称" value="${jobdef?if_exists.name?if_exists}" readonly=true/>
        <#else>
        <@c.FormInput name="name" label="名称" value="${jobdef?if_exists.name?if_exists}" helper="1-100位字符,仅支持汉字、字母、数字和部分特殊字符（.- _()[]（）【】）组合" events="{blur :function(){$Utils.validInputSpeChar(this)}}" validation={"alphanumerSpec":true,"required":true,"minlength":1,"maxlength":100,"remote":"${ContextPath}/schedule/alertdef/synchroCheck.d"} messages={"remote":"名称不能重复，请确认！"} />
        </#if>
        ...
        <@c.FormText name="description" label="描述" validation={"maxlength":250}>${jobdef?if_exists.description?if_exists}</@c.FormText>
    ...
    <@c.BoxFooter class="text-center">
        <@c.Button type="primary" icon="fa fa-save" action=[c.saveform("#alertdefForm")]>保存</@c.Button>
