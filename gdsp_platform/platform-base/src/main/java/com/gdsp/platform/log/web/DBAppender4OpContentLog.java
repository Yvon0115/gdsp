...
/**
 * 扩展DBAppenderBase<ILoggingEvent>，记录操作日志内容
 * @author gy
 *
 */
public class DBAppender4OpContentLog extends DBAppenderBase<ILoggingEvent> {

    protected String               insertSQL;
    protected static final Method  GET_GENERATED_KEYS_METHOD;
    static final StackTraceElement EMPTY_CALLER_DATA = CallerData.naInstance();
    private static final Logger logger = LoggerFactory.getLogger(DBAppender4OpContentLog.class);
    static {
    ...
    @Override
    public void start() {
        insertSQL = "insert into cp_log_content(id,log_id,col_name,col_desc, new_data,old_data) values("
                + "?,?,?,?,?,?)";
        super.start();
    }
