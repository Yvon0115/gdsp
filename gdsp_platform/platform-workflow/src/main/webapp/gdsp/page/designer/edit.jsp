...
<head>
	...
	<title>流程设计器</title>
	<!-- JQuery EasyUi CSS-->
	<link type="text/css" href="${ctx }/gdsp/js/wfdesignerjs/jquery-easyui/themes/default/easyui.css" rel="stylesheet" title="blue">
...
function saveProcessDef(){	
	...
	jq.ajax({
		...
		success:function(data){
			if(data){
				jq.messager.alert('Info','流程定义保存成功!','info');
				setTimeout("closeIndex()",2000);
			}else{
				jq.messager.alert('Error','流程定义保存失败!','error');
			}
		},
		error:function(){
			jq.messager.alert('Info','error','info');
			return "";
		}
	}); 
...
//流程模型回显
function parseProcessDescriptor(xmlDef,jsonDef){
	var descriptor = jq(xmlDef);
	...
	//json回显流程属性
	workflow.process.flowcategory=json.flowcategory;
	workflow.process.flowcategoryname=json.flowcategoryname;
	//workflow.process.tableID=json.tableID;
	workflow.process.form=json.form;
	...
	userTasks.each(function(i){
			...
			//通过json串回显task数据
			for(var i=0;i<json.task.length;i++){
				if(tid==json.task[i].taskId){
					//1.回显节点属性
					task.form=json.task[i].form;
					task.emailinform=json.task[i].emailinform;
					task.overtime=json.task[i].overtime;
					task.unit=json.task[i].unit;
					task.workingday=json.task[i].workingday;
					task.dealMethod=json.task[i].dealMethod;
					task.extendEvent=json.task[i].extendEvent;
					task.systeminform=json.task[i].systeminform;
					//2.回显办理人
					for(var j=0;j<json.task[i].candidateUsers.length;j++){
						if(json.task[i].candidateUsers[j].type=="user"){
							task.addCandidateUser({
								userId:json.task[i].candidateUsers[j].id,
								userName:json.task[i].candidateUsers[j].name,
								userCode:json.task[i].candidateUsers[j].code
							});
						}
						if(json.task[i].candidateUsers[j].type=="group"){
							task.addCandidateGroup({
								groupId:json.task[i].candidateUsers[j].id,
								groupName:json.task[i].candidateUsers[j].name,
								groupCode:json.task[i].candidateUsers[j].code
							});
						}
						if(json.task[i].candidateUsers[j].type=="role"){
							task.addCandidateRole({
								roleId:json.task[i].candidateUsers[j].id,
								roleName:json.task[i].candidateUsers[j].name,
								roleCode:json.task[i].candidateUsers[j].code
							});
						}
						if(json.task[i].candidateUsers[j].type=="org"){
							task.addCandidateOrg({
								orgId:json.task[i].candidateUsers[j].id,
								orgName:json.task[i].candidateUsers[j].name,
								orgCode:json.task[i].candidateUsers[j].code
							});
						}
						
					}
					//3.回显会签属性
					task._multiInstanceRule=json.task[i].multiInsRule;
					task._multiInstanceVal=json.task[i].multiInsVal;
				}
			}
			
			//回显会签属性
			var multiInstanceLoopCharacteristics = jq(this).find("multiInstanceLoopCharacteristics");
			task.isSequential = multiInstanceLoopCharacteristics.attr("isSequential");
			/* if(multiInstanceLoopCharacteristics!=null && multiInstanceLoopCharacteristics != ""){
				task._loopCardinality = multiInstanceLoopCharacteristics.find("loopCardinality").text();
				task._collection = multiInstanceLoopCharacteristics.attr("activiti:collection");
				task._elementVariable = multiInstanceLoopCharacteristics.attr("activiti:elementVariable");
				task._completionCondition = multiInstanceLoopCharacteristics.find("completionCondition").text();
			} */
...
<body id="designer" class="easyui-layout">
	...
	<script type="text/javascript">
		...
		function openProcessDef(){
			...
			var jsonDef = "";
			jq.ajax({
				url:"${ctx }/workflow/model/getXml.d",
				type: 'POST',
				async: false,
				data : {
					modelId : "<%=modelId%>"
			...
			jq.ajax({
				url:"${ctx }/workflow/model/getJson.d",
				type: 'POST',
				async: false,
				data : {
					modelId : "<%=modelId%>"
			...
			if(jsonDef!=""&&xmlDef!=""){
				parseProcessDescriptor(xmlDef,jsonDef);
			}
		}
			...
			try{
				...
				if(typeof processDefinitionId != "undefined" && processDefinitionId != null &&  processDefinitionId != "null" && processDefinitionId != "" && processDefinitionId != "NULL"){
					openProcessDef();
						...
						workflow.process.tableID="<%=tableID%>";
