...
draw2d.MyCanvas.prototype.toJSON=function(){		
	var processJson='';
	processJson=processJson+"processName:'"+this.process.name+"',";
	processJson=processJson+"processID:'"+this.process.id+"',";
	processJson=processJson+"form:'"+this.process.form+"',";
	processJson=processJson+"flowcategory:'"+this.process.flowcategory+"',";
	processJson=processJson+"flowcategoryname:'"+this.process.flowcategoryname+"',";
	processJson=processJson+"documentation:'"+this.process.documentation+"',";
	processJson=processJson+"tableID:'"+this.process.tableID+"',";
	var json='task:[';				
	...
	for(var i=0;i<models.getSize();i++){
		...
		for(var j=0;j<DefaultModelTypeEnum.length;j++){
			if(DefaultModelTypeEnum[j]==model.type){
				if(model.type.toString()=="draw2d.UserTask"){
					json += "{";
					json += model.toJson()+"},";
				}
				break;
