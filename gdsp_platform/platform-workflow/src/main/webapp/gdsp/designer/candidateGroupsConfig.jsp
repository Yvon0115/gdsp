...
<!--
	jq(function() {
		...
		_task_candidate_groups_list = jq('#task-candidate-groups-list')
				.datagrid(
						{
							title : "候选用户组",
							singleSelect : true,
							...
							toolbar : [ {
								...
								handler : function() {
									...
									_task_candidate_win.window('setTitle',
											'Candidate User Config');
									_task_candidate_win.window('refresh',
											'${ctx }/designer/addCandidateGroup.jsp');
								}
							} ]
						});
	...
	function candidateGroupsBtFormatter(value, rowData, rowIndex) {
		...
		var d = '<img onclick="deleteCandidateGroup(\'' + name
	...
	function loadTaskCandidateGroups() {
		...
		_task_candidate_groups_list.datagrid('loadData',
				candidate_groups_grid_data);
	}
//-->
</script>
...
<table id="task-candidate-groups-list" name="task-candidate-groups-list">
	<thead>
		<tr>
			<th field="groupName" width="100" align="middle" sortable="false">名称</th>
			<th field="groupId" width="100" align="middle" sortable="false" hidden="hidden">id</th>
			<th field="action" width="80" align="middle"
				formatter="candidateGroupsBtFormatter">操作</th>
		</tr>
	</thead>
</table>
</div>