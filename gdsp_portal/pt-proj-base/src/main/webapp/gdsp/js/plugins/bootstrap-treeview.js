...
;(function($, window, document, undefined) {
	...
	Tree.prototype = {
		...
		_render: function() {
			...
			if(self.options.onRender&&typeof(self.options.onRender)==='function'){
				self.options.onRender();
			}
		},
		...
		_buildTree: function(nodes, level) {
			...
			$.each(nodes, function addNodes(id, node) {
				...
				if(!node.isdir){
					if(node.page_type!=null&&node.page_id!=null){
						treeItem.attr('href',__contextPath+'/appcfg/pagecfg/pageConfig.d?page_id='+node.page_id);
						treeItem.attr('data-toggle','activetab');
						treeItem.attr('data-target','#configPanel');
						treeItem.attr('reload','true');
					}
					treeItem.attr('page_type',node.page_type);
					treeItem.attr('page_id',node.page_id);
				}
				...
				if (node._nodes) {
					...
					treeItem
						.append($(self._template.expandCollapseIcon)
							.addClass(node.isdir?node.icon:node.icon)
						);
				}

				// Add node icon
				// Add folder, folder open or leaf icons
				// to facilitate tree structure navigation
				if (node._nodes) {
					treeItem
						.append($(self._template.icon)
							.addClass(self.options.folderIcon)
						);
				}
				else if (node.nodes) {
					treeItem
						.append($(self._template.icon)
							.addClass(self.options.folderOpenIcon)
						);
				}
				else if(node.isdir) {
					treeItem
		...
		_template: {
			//href="'+__contextPath+'"/appcfg/pagecfg/deleteDir.d"
			list: '<ul class="list-group"></ul>',
			item: '<li class="list-group-item" onmouseover="_showbtn(true,this)" onmouseout="_showbtn(false,this)"></li>',
			indent: '<span class="indent"></span>',
			expandCollapseIcon: '<span class="expand-collapse"></span>',
			icon: '<span class="icon"></span>',
			link: '<a href="#" style="color:inherit;"></a>',
			badge: '<div style="background-color: transparent;float:right;margin-left:5px"></div>'
		},

		_css: '.treeview .list-group-item{cursor:pointer}.treeview span.indent{margin-left:10px;margin-right:10px}.treeview span.expand-collapse{width:1rem;height:1rem}.treeview span.icon{margin-left:0px;margin-right:5px}'
