...
<@c.Box>
    ...
    <@c.BoxBody >
	    <@c.Form id="paramForm" class="validate" action="${ContextPath}/widgetmgr/param/save.d" method="post" after={"switchtab":"#mainParamPanel","dataloader":"#paramName"}>
	        <@c.FormIdVersion id="${param?if_exists.id?if_exists}" version="${param?if_exists.version?default(0)}"/>
           <@c.FormInput name="name" label="参数名称" value="${param?if_exists.name?if_exists}" validation={"required":true}/>
	         <@c.FormInput name="code" label="参数Key" value="${param?if_exists.code?if_exists}" validation={"required":true} helper="需与报表接收参数一致。"/>
	          <!-- 手工，SQL， 文件 -->
	       ...
	       <@c.FormText itemId="data_source_div" id="data_source" name="data_source" label="数据源内容"  style="display:block" rows=3 >${param?if_exists.data_source?if_exists}</@c.FormText>
	        ...
	        <!--<@c.FormCheckBox itemId="cube_flag_div" name="cube_flag" label="cube参数标志" value="${widget?if_exists.cube_flag?if_exists}"/>-->
    ...
    <@c.BoxFooter class="text-center">
        <@c.Button type="primary"  icon="fa fa-save" action=[c.saveform("#paramForm")]>保存</@c.Button>
        <@c.Button type="canel" icon="glyphicon glyphicon-off" action=[c.opentab("#mainParamPanel")]>取消</@c.Button>
    </@c.BoxFooter>
</@c.Box>
