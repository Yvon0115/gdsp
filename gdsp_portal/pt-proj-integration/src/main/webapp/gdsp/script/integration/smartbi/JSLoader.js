...
define([ "cas/modal" ], function() {
	var funcs = {
			loginSmartbi: function(){
			    //smartbi服务器的URL地址

			    var config = new Object();

			    config.baseURL = "http://localhost:18080/smartbi/vision/";

			    var BOF_UI_DEBUG = false;

			    // 创建全局唯一的JS装载器

			    var jsloader = this.JSLoader(config);
				...
				var Getter = function() {
					...
					return {
						...
						resolve : function(name, useGlobal) {
							var bofClassName = name, url = null, useGbk = useGlobal === true ? false : true;
							...
							var __url = url;
							url = baseURL + (useGbk ? 'gbk.jsp?name=vision/' : '') + url;
							if (config.lang)
