...
<@c.Box>
    ...
    <@c.BoxBody>
		...
		<@c.FormItem colspan=2>
	        ...
	        <@c.FormInput id="operatorid" name="operatorid" label="租户管理员id"   value="${dateSourceVO?if_exists.operatorid?if_exists}" validation={"required":true,"minlength":1,"maxlength":50} helper="1-50个字符,访问阿里系统的账号的的租户管理员id"/>
	        <@c.FormInput id="username" name="username" label="账号"   value="${dateSourceVO?if_exists.username?if_exists}" validation={"required":true,"minlength":1,"maxlength":50} class="userInfo" helper="1-50个字符,访问阿里报表系统时提供的账号"/>
