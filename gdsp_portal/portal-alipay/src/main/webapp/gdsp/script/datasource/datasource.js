...
function inputDispalyControl(container,dsType,value){
	...
	// port显示控制
	var noPort = new Array("birt","cognos","smartbi","Teradata","alisis");
...
//	if($.inArray(value, arr1) != -1 || "DBTYpe" == dsType){
	...
	if(value == "birt" || (value == "HuaweiHive" && model == "2") || value == "alisis"){
	...
	if(dsType=="BIType"){
		if(value == "alisis"){
			...
			$("#path").attr("value",'ALISIS_DEFAULT');
		}else{
	...
	//租户id和租户管理员id显示控制
	if(value == "alisis"){
		$(container +"-tabpane #operatorid").parents(".form-group:first").show();
	}else{
		$(container +"-tabpane #operatorid").parents(".form-group:first").hide();
