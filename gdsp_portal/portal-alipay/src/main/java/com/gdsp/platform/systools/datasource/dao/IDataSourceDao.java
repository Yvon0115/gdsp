...
public interface IDataSourceDao {
    ...
    * @return void    返回值说明
    *      */
    @Insert("insert into pt_datasource (id ,code,name ,username,password,type,comments,driver,ip,port,url,default_flag,span,permissionurl,isDefault,createtime,createby,version,px_url,path,product_version,authentication_model,keytabPath,krbConfPath,classify,operatorid) "
            + "values (#{id},#{code},#{name},#{username},#{password},#{type},#{comments},#{driver},#{ip},#{port},#{url},#{default_flag},#{span},#{permissionurl},#{isDefault},#{createTime:BIGINT},#{createBy},#{version},#{px_url},#{path},#{product_version},#{authentication_model:BIGINT},#{keytabPath},#{krbConfPath},#{classify},#{operatorid})")
