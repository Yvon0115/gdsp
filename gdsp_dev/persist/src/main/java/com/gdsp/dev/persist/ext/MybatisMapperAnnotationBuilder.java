...
public class MybatisMapperAnnotationBuilder extends MapperAnnotationBuilder {
     ...
     * 日志对象
     */
    private static Logger      logger          = LoggerFactory.getLogger(MybatisMapperAnnotationBuilder.class);
    private final Set<Class<? extends Annotation>> sqlAnnotationTypes         = new HashSet<Class<? extends Annotation>>();
    private final Set<Class<? extends Annotation>> sqlProviderAnnotationTypes = new HashSet<Class<? extends Annotation>>();
    /**
     * Mybatis configuration object ,that is platform custom extension of Configuration.class
     */
    private MybatisConfiguration                   configuration;
    /**
     * Assistant of mybatis building mapping config
     */
    ...
    private Class<?>                               type;
    /**
     * Mybatis dao class user datasource name
     */
    private String                                 typeDataSourceId           = null;

    /**
     * Construct method
     * @param configuration mybatis config object
     * @param type dao class
     */
    ...
    public void parse() {
        ...
        if (!configuration.isResourceLoaded(resource)) {
            ...
            if (ds != null && StringUtils.isNotEmpty(ds.value())) {
                typeDataSourceId = ds.value();
            }
            for (Method method : methods) {
                try {
                    parseMethodStatement(method);
                } catch (IncompleteElementException e) {
                    configuration.addIncompleteMethod(new MethodResolver(this,
                            method));
    ...
    private void parsePendingMethods() {
        Collection<MethodResolver> incompleteMethods = configuration
    ...
    private void parseCache() {
        ...
        if (cacheDomain != null) {
            assistant.useNewCache(cacheDomain.implementation(),
                    cacheDomain.eviction(), cacheDomain.flushInterval(),
                    cacheDomain.size(), cacheDomain.readWrite(), null);
        }
    }

    private void parseCacheRef() {
        CacheNamespaceRef cacheDomainRef = type
                .getAnnotation(CacheNamespaceRef.class);
    ...
    private String parseResultMap(Method method) {
        ...
        applyResultMap(resultMapId, returnType, argsIf(args),
    ...
    private void applyResultMap(String resultMapId, Class<?> returnType,
            Arg[] args, Result[] results, TypeDiscriminator discriminator) {
        ...
        assistant.addResultMap(resultMapId, returnType, null, disc,
                resultMappings, null); // TODO add AutoMappingBehaviour
        createDiscriminatorResultMaps(resultMapId, returnType, discriminator);
    }

    private void createDiscriminatorResultMaps(String resultMapId,
            Class<?> resultType, TypeDiscriminator discriminator) {
        if (discriminator != null) {
            for (Case c : discriminator.cases()) {
                String caseResultMapId = resultMapId + "-" + c.value();
                List<ResultMapping> resultMappings = new ArrayList<ResultMapping>();
                applyConstructorArgs(c.constructArgs(), resultType,
                        resultMappings); // issue #136
                applyResults(c.results(), resultType, resultMappings);
                assistant.addResultMap(caseResultMapId, c.type(), resultMapId,
                        null, resultMappings, null); // TODO add
                                                                                                                    // AutoMappingBehaviour
            }
        }
    }

    private Discriminator applyDiscriminator(String resultMapId,
            Class<?> resultType, TypeDiscriminator discriminator) {
        if (discriminator != null) {
            ...
            Class<? extends TypeHandler<?>> typeHandler = discriminator
    ...
    protected void parseMethodStatement(final Method method) {
        ...
        final String mappedStatementId = type.getName() + "."
                + method.getName();
        buildMethodDataSource(mappedStatementId, method);
        if (sqlSource != null) {
            ...
            if (SqlCommandType.INSERT.equals(sqlCommandType)
                ...
                if (selectKey != null) {
                    keyGenerator = handleSelectKeyAnnotation(selectKey,
                            mappedStatementId, getParameterType(method),
                            languageDriver);
            ...
            if (options != null) {
                ...
                fetchSize = options.fetchSize() > -1
                        || options.fetchSize() == Integer.MIN_VALUE ? options
            ...
            assistant.addMappedStatement(mappedStatementId, sqlSource,
                    ...
                    getReturnType(method), resultSetType, flushCache,
                    useCache,
    ...
    /**
     * Build dao method using datasource config
     * @param mappedStatementId mappedconfig id
     * @param method dao method object
     */
    private void buildMethodDataSource(String mappedStatementId, Method method) {
        DataSource ds = method.getAnnotation(DataSource.class);
        String dsName = typeDataSourceId;
        if (ds != null && StringUtils.isNotEmpty(ds.value())) {
            dsName = ds.value();
        }
        if (dsName == null) {
            configuration.putDataSource(mappedStatementId, dsName);
        }
    }
    ...
    private Class<?> getReturnType(Method method) {
        ...
        if (void.class.equals(returnType)) { // issue #508
        ...
        } else if (configuration.getObjectFactory().isCollection(returnType)) {
            ...
            // (issue 504) Do not look into Maps if there is not MapKey
            // annotation
            Type returnTypeParameter = method.getGenericReturnType();
            if (returnTypeParameter instanceof ParameterizedType) {
                Type[] actualTypeArguments = ((ParameterizedType) returnTypeParameter)
    ...
    private SqlSource getSqlSourceFromAnnotations(Method method,
        ...
        try {
            ...
            if (sqlAnnotationType != null) {
                ...
                Annotation sqlAnnotation = method
                        .getAnnotation(sqlAnnotationType);
                ...
                Annotation sqlProviderAnnotation = method
                        .getAnnotation(sqlProviderAnnotationType);
    ...
    private SqlSource buildSqlSourceFromStrings(String[] strings,
        ...
        return languageDriver.createSqlSource(configuration, sql.toString(),
                parameterTypeClass);
    }
    ...
    private Class<? extends Annotation> getSqlProviderAnnotationType(
            Method method) {
    ...
    private void applyResults(Result[] results, Class<?> resultType,
        ...
        for (Result result : results) {
            ...
            ResultMapping resultMapping = assistant.buildResultMapping(
                    ...
                    result.jdbcType() == JdbcType.UNDEFINED ? null : result
                            .jdbcType(),
                    hasNestedSelect(result) ? nestedSelectId(result) : null,
                    null, null, null,
                    result.typeHandler() == UnknownTypeHandler.class ? null
                            : result.typeHandler(),
                    flags, null, null,
                    isLazy(result));
    ...
    private void applyConstructorArgs(Arg[] args, Class<?> resultType,
        ...
        for (Arg arg : args) {
            ...
            ResultMapping resultMapping = assistant.buildResultMapping(
                    ...
                    arg.jdbcType() == JdbcType.UNDEFINED ? null : arg
                            .jdbcType(),
                    nullOrEmpty(arg.select()),
                    nullOrEmpty(arg.resultMap()),
                    null,
                    null,
                    arg.typeHandler() == UnknownTypeHandler.class ? null : arg
                            .typeHandler(),
                    flags, null, null, false);
            resultMappings.add(resultMapping);
        }
    }
    ...
    private KeyGenerator handleSelectKeyAnnotation(
        ...
        assistant.addMappedStatement(id, sqlSource, statementType,
                ...
                resultSetTypeEnum, flushCache, useCache, false, keyGenerator,
